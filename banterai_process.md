BanterAI — extension 作業流程說明
=================================

目的
----
記錄 BanterAI extension 的作業流程、設定結構與 AI + 使用者互動的行為規範。這個文件不會包進 extension 內，是團隊用來定義流程與 config 格式的參考。
...
----------------------
1. Extension 啟動時：
   - 讀取 `.banterai/config.json`（若不存在，建立預設結構）。
...
2. 當 AI 呼叫該 tool 時：
   - tool 讀取 `.banterai/config.json` 中的 `team`，尋找 `judy`（不區分大小寫或用更嚴格的對應規則）。
...
3. 若無法註冊 tool（例如平台不允許或 sandbox 限制，如 Antigravity），則：
   - 把原本 tool 需要執行的步驟與必要資料記錄到 `.banterai/README.md`（或 `TODO` 區塊），讓 AI/使用者看到該如何手動執行或授權外部服務執行。
...
安全與隱私建議
----------------
- 明確告知使用者哪些 key 會儲存在 `.banterai/config.json`，並提供移除/更新介面。
- 若要支援團隊共用設定，考慮把敏感值留在使用者本地，或提供安全儲存（例如 OS keychain）選項。

變更與維護
----------------
- 當流程或資料欄位改動時，更新此文件並在 `.banterai/README.md` 加上變更日誌。
