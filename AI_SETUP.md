# BanterAI AI 設定指南 (AI Configuration Guide)

本指南旨在協助 AI 助理理解如何引導用戶設定 BanterAI Extension。
...
*   **來源優先順序**:
    ...
    2.  若無 Git 設定，詢問用戶：「您希望在 BanterAI 中使用什麼暱稱？」
...
*   **設定位置**: 寫入 VS Code 設定 `banterAI.ablyApiKey`。
...
*   **設定位置**: 寫入 VS Code 設定 `banterAI.cipherKey` (建議不 Sync 到 Git)。
...
1.  **環境掃描**:
    ...
    *   檢查 `.vscode/settings.json` 是否已有 BanterAI 相關設定。

2.  **互動對話 (Example)**:
    > **AI**: "嗨！歡迎使用 BanterAI。我來幫您設定連線資訊：
    > 1.  我偵測到您的 Git 名字是 `Eric`，頻道將設為 `ericchen1972/BanterAI`，這樣可以嗎？
...
3.  **執行設定**:
    *   根據用戶回覆，將設定值寫入 `.vscode/settings.json`：
        ```json
        {
            "banterAI.nickname": "Eric",
            "banterAI.channelName": "ericchen1972/BanterAI",
            "banterAI.ablyApiKey": "YOUR_ABLY_KEY_HERE",
            "banterAI.cipherKey": "SECRET_PASSWORD_HERE" // 若有
        }
        ```

4.  **完成通知**:
    > **AI**: "設定完成！BanterAI 已啟動。您現在應該能看到線上隊友了。試著在聊天室說聲 Hello 吧！"
...
*   **人設查找**:
    1.  若是**私訊**，到 `.banterai/config.json` 查找該人名的 `special_prompt`。若有（例如：跟 Judy 說話可以油膩一點），請以此風格改寫原始訊息。
...
*   **提示**: 由於 BanterAI 已經有內建的聊天文字輸入框供使用者親自「直接傳送訊息 (Direct Relay)」，因此所有呼叫您的請求，都代表使用者希望您發揮 AI 的專長幫忙「修飾/傳達」！

### C. 執行指令 (Tool / CLI Call)
當 AI 確定要傳送訊息時，這是執行的優先順序：
1. **優先使用原生 Tool API (推薦 Codex / Copilot 使用)**: 
   如果您支援 Language Model Tools API，請直接呼叫已註冊的 `banterai_send_message` 工具，並傳入 `to` 與 `content`。
2. **終端機降級方案 (推薦 Cursor / Antigravity 使用)**: 
   如果您看不到上述 Tool，請執行 `run_command` 呼叫 CLI 腳本：
   `node ./out/cli.js --to "[收件人]" --content "[訊息內容]"`
3. **無動作方案 (Fallback)**: 
   如果前兩者都無法使用，請生成最終文字並引導用戶使用下方的 BanterAI 面板點擊發送按鈕。
